...
parser.add_argument('--dataset', type=str, choices=['cifar10', 'cifar100', 'imagenet', 'svhn', 'stl10', 'papcell'], help='Choose between Cifar10/100 and ImageNet.')
...
def main():
  ...
  #add svhn datasets
    ...
    mean = [x / 255 for x in [112.5945, 113.7447, 120.3092]] #need to change
    std = [x / 255 for x in [52.0105, 53.0540, 52.4688]] #need to change 
  #add papcell datasets
  elif args.dataset == 'papcell':
    mean = [x / 255 for x in [0, 0, 0]] #need to change
    std = [x / 255 for x in [1, 1, 1]] #need to change 
  else:
    assert False, "Unknow dataset : {}".format(args.dataset)

  train_transform = transforms.Compose(
    [transforms.RandomHorizontalFlip(), transforms.RandomResizedCrop(227), transforms.ToTensor(),
  ...
  #add papcell datasets
    ...
    test_data = dset.ImageFolder(valdir, test_transform)               
    num_classes = 7
  ...
  miniTrainSet=[]
  for i in range(1000):
      miniTrainSet.append(train_data[i])

  train_loader = torch.utils.data.DataLoader(miniTrainSet, batch_size=args.batch_size,
                                            shuffle=True, num_workers=2) #加载训练集,每次加载4张图片
  #用100张图片做模拟测试，看能否在这个测试集中的效果
  miniTestSet=[]
  for i in range(100):
      miniTestSet.append(test_data[i])
  test_loader = torch.utils.data.DataLoader(miniTestSet, batch_size=args.batch_size,
                                          shuffle=False, num_workers=2) #加载测试集,每次加载4张图片
################################模拟训练集和测试集################################################  
  ...
  if args.resume:
    if os.path.isfile(args.resume):
      ...
      checkpoint = torch.load(args.resume) #args,resum is directory of chckpoint.pth.tar
  ...
  for epoch in range(args.start_epoch, args.epochs):
    ...
    save_checkpoint({
      ...
      'recorder': recorder,  #all data are saved in recorder
